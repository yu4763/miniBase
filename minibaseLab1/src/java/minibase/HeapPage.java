...
public class HeapPage implements Page {
	...
	public void deleteTuple(Tuple t) throws DbException {
		...
		while (it.hasNext()) {
			Tuple temp = it.next();
			if (temp.equals(t)) {
				int i = temp.getRecordId().tupleno();

				if (!isSlotUsed(i))
					throw new DbException("tuple not exist");

				markSlotUsed(i, false);
				temp.setRecordId(null);
				exist = true;
				break;
	...
	public void insertTuple(Tuple t) throws DbException {
		...
		if (getNumEmptySlots() == 0)
			throw new DbException("thie page is full");
	...
	public boolean isSlotUsed(int i) {
		// TODO: some code goes here
		Byte check = header[i / 8];
		return ((check >> i % 8) & 1) == 1;
	}
	...
	private void markSlotUsed(int i, boolean value) {
		...
		int mask = 1 << i % 8;
		check = (byte) ((check & ~mask) | ((v << i % 8) & mask));
	...
	public Iterator<Tuple> iterator() {
		...
		Tuple[] temp = new Tuple[numSlots - getNumEmptySlots()];

		for (int i = 0, k = 0; i < numSlots; i++) {
			if (isSlotUsed(i))
				temp[k++] = tuples[i];
		}
