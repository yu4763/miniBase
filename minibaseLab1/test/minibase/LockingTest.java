...
public class LockingTest extends TestUtil.CreateHeapFile {
	 ...
	 * @param expected true if we expect the second acquisition to succeed; false
	 *                 otherwise
	 */
	public void metaLockTester(TransactionId tid1, PageId pid1, Permissions perm1, TransactionId tid2, PageId pid2,
			Permissions perm2, boolean expected) throws Exception {
	...
	/**
	 * Generic unit test structure to grab an additional lock in a new thread.
	 *
	 * @param tid      the transaction Id
	 * @param pid      the first page to lock over
	 * @param perm     the type of lock desired
	 * @param expected true if we expect the acquisition to succeed; false otherwise
	 */
	public void grabLock(TransactionId tid, PageId pid, Permissions perm, boolean expected) throws Exception {
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires two read locks
	 * on the same page.
	 */
	@Test
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires a read lock and
	 * a write lock on the same page, in that order.
	 */
	@Test
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires a write lock
	 * and a read lock on the same page, in that order.
	 */
	@Test
	public void acquireWriteReadLocksOnSamePage() throws Exception {
		metaLockTester(tid1, p0, Permissions.READ_WRITE, tid2, p0, Permissions.READ_ONLY, false);
	}

	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires a read lock and
	 * a write lock on different pages.
	 */
	@Test
	public void acquireReadWriteLocksOnTwoPages() throws Exception {
		metaLockTester(tid1, p0, Permissions.READ_ONLY, tid2, p1, Permissions.READ_WRITE, true);
	}

	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires write locks on
	 * different pages.
	 */
	@Test
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. Acquires read locks on
	 * different pages.
	 */
	@Test
	public void acquireReadLocksOnTwoPages() throws Exception {
		metaLockTester(tid1, p0, Permissions.READ_ONLY, tid2, p1, Permissions.READ_ONLY, true);
	}
	...
	/**
	 * Unit test for BufferPool.getPage() assuming locking. A single transaction
	 * should be able to acquire a read lock after it already has a write lock.
	 */
	@Test
	public void acquireWriteAndReadLocks() throws Exception {
		metaLockTester(tid1, p0, Permissions.READ_WRITE, tid1, p0, Permissions.READ_ONLY, true);
	}

	/**
	 * Unit test for BufferPool.getPage() and BufferPool.releasePage() assuming
	 * locking. Acquires read locks on different pages.
	 */
	@Test
